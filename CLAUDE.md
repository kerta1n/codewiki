...
skills/
  codewiki-session.md — Claude Code skill file (embedded via include_str! in setup.rs)
...
- **Skill is embedded** in the binary via `include_str!`. Editing `skills/codewiki-session.md` requires recompiling to update installed skills.
...
| `cw setup claude-code` | Install skill to `~/.claude/skills/codewiki/SKILL.md` |
